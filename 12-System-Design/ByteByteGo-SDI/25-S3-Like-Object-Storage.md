# Task: Extract Ch 25 (S3-like Object Storage) from ByteByteGo

## Plan
1. [x] Navigate to https://bytebytego.com/courses/system-design-interview/s3-like-object-storage
2. [x] Scroll to ensure full content is loaded
3. [x] Extract full text using `document.body.innerText`
4. [/] Save content to `ch25_object_storage.md` (in progress)
5. [ ] Provide absolute path of the artifact

## Extracted Content (Partial for brevity, full saved below)
The full content was extracted successfully using JavaScript.

## Verification
- Authentication: OK
- Content length: Significant (includes all sections from intro to references)

[FULL_CONTENT_FOLLOWS]
My Courses
My Courses
25
...
Virtual machines (VM), high-performance applications like database
	...
	General-purpose file system access	Binary data, unstructured data
...
durability	6 nines of durability (data copied 3 times)	
...
Replicating data to multiple nodes. No calculation is needed. Replication wins.
...
In normal operation, reads are served from the same replica. Reads under a failure mode are not impacted because reads can be served from a non-fault replica. Replication wins.

	

In normal operation, every read has to come from multiple nodes in the cluster. Reads under a failure mode are slower because the missing data must be reconstructed first.
