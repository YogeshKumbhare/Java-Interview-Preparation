...
> Source: [ByteByteGo - System Design Interview](https://bytebytego.com/courses/system-design-interview/scale-from-zero-to-millions-of-users)
...
![Figure 1 – Single Server Setup with DNS](images/ch02/figure-1.png)

*Figure 1: A simplified client-server architecture with DNS resolution. A user with a web browser and mobile app sends requests through DNS to a single web server.*
...
![Figure 2 – Request Flow](images/ch02/figure-2.png)

*Figure 2: The request flow showing DNS resolution and HTTP request/response cycle.*
...
### Java Example – Simple REST API Controller (Spring Boot)
...
![Figure 3 – Web Tier and Data Tier](images/ch02/figure-3.png)

*Figure 3: Separating web server and database server into web tier and data tier.*
...
### Java Example – JDBC vs NoSQL Connection

```java
// === Relational DB (JDBC with MySQL) ===
import java.sql.*;
...
public class RelationalDbExample {
    public static void main(String[] args) throws Exception {
        ...
        while (rs.next()) {
            System.out.println("User: " + rs.getString("first_name") 
...
```java
// === NoSQL (MongoDB with Java Driver) ===
import com.mongodb.client.*;
import org.bson.Document;

public class NoSqlDbExample {
    public static void main(String[] args) {
        MongoClient mongoClient = MongoClients.create("mongodb://localhost:27017");
        MongoDatabase database = mongoClient.getDatabase("mydb");
        MongoCollection<Document> collection = database.getCollection("users");
        
        Document user = collection.find(new Document("id", 12)).first();
        System.out.println("User: " + user.getString("firstName")
...
![Figure 4 – Load Balancer](images/ch02/figure-4.png)

*Figure 4: Load balancer distributing traffic between Server1 and Server2 using private IPs.*
...
### Java Example – Round-Robin Load Balancer Simulation
...
public class RoundRobinLoadBalancer {
    ...
    public static void main(String[] args) {
        RoundRobinLoadBalancer lb = new RoundRobinLoadBalancer(
            List.of("10.0.0.1:8080", "10.0.0.2:8080", "10.0.0.3:8080")
        );
...
![Figure 5 – Master-Slave Replication](images/ch02/figure-5.svg)

*Figure 5: Master-slave database replication. Web servers write to Master DB and read from Slave DBs.*
...
![Figure 6 – Load Balancer + DB Replication](images/ch02/figure-6.png)

*Figure 6: Complete architecture with load balancer, web tier (Server 1 & 2), and data tier (Master DB & Slave DB).*
...
### Java Example – Read/Write Splitting with Spring Boot

```java
// DataSource routing for Master/Slave split
...
```java
// Service layer using @Transactional for read/write routing
@Service
public class UserService {
    ...
    // Reads go to SLAVE (readOnly = true)
...
![Figure 7 – Cache Tier](images/ch02/figure-7.svg)

*Figure 7: Read-through cache strategy. Web server checks cache first, then falls back to database.*
...
Interacting with cache servers is simple because most cache servers provide APIs for common programming languages. The following code snippet shows typical cache interaction:

### Java Example – Redis Cache with Spring Boot + Jedis

```java
// Using Jedis (Redis Java client) directly
...
public class CacheExample {
    ...
    public static void main(String[] args) {
        try (Jedis jedis = new Jedis("localhost", 6379)) {
            // SET with expiration
            jedis.setex("myKey", CACHE_TTL_SECONDS, "hi there");
            
            // GET from cache
            String value = jedis.get("myKey");
            System.out.println("Cached value: " + value); // "hi there"
        }
    }
}
```
...
```java
// Using Spring Boot @Cacheable (recommended for production)
@Service
public class UserService {

    @Cacheable(value = "users", key = "#id")
    public User getUserById(Long id) {
        // This method is only called if data is NOT in cache.
        // On subsequent calls, the cached result is returned directly.
...
![Figure 8 – Single Point of Failure](images/ch02/figure-8.png)

*Figure 8: Single Point of Failure – Three users connected to a single server.*

- **Eviction Policy:** Once the cache is full, any requests to add items to the cache might cause existing items to be removed. This is called cache eviction. Least-recently-used (LRU) is the most popular cache eviction policy. Other eviction policies, such as the Least Frequently Used (LFU) or First in First Out (FIFO), can be adopted to satisfy different use cases.

### Java Example – Simple LRU Cache Implementation
...
public class LRUCache<K, V> extends LinkedHashMap<K, V> {
    ...
    public LRUCache(int capacity) {
        // accessOrder = true means LRU ordering
...
![Figure 9 – CDN Latency](images/ch02/figure-9.svg)

*Figure 9: CDN reduces latency – 30ms via CDN vs 120ms direct to origin.*

Figure 10 demonstrates the CDN workflow.

![Figure 10 – CDN Workflow](images/ch02/figure-10.png)

*Figure 10: CDN workflow – User A requests image, CDN fetches from origin if not cached, User B gets cached version.*
...
![Figure 11 – CDN + Cache Architecture](images/ch02/figure-11.png)

*Figure 11: Complete architecture with CDN, cache, load balancer, web tier, and data tier.*
...
![Figure 12 – Stateful Architecture](images/ch02/figure-12.png)

*Figure 12: Stateful architecture – Each user's session data is stored on a specific server.*
...
![Figure 13 – Stateless Architecture](images/ch02/figure-13.png)

*Figure 13: Stateless architecture – All web servers fetch state from a shared data store.*
...
### Java Example – Stateless Session with Redis (Spring Session)
...
// Spring Session automatically stores HttpSession in Redis
@RestController
...
![Figure 14 – Stateless Web Tier Design](images/ch02/figure-14.png)

*Figure 14: Updated design with stateless web tier, auto-scaling, and NoSQL shared data store.*
...
![Figure 15 – Multi Data Center](images/ch02/figure-15.png)

*Figure 15: Multi-data center setup with geo-routing between US-East and US-West.*
...
![Figure 16 – Data Center Failover](images/ch02/figure-16.png)

*Figure 16: Failover scenario – All traffic routed to US-East when US-West is down.*
...
![Figure 17 – Message Queue](images/ch02/figure-17.svg)

*Figure 17: Producer-Consumer architecture with a message queue.*
...
![Figure 18 – Photo Processing Queue](images/ch02/figure-18.svg)

*Figure 18: Photo processing using message queue – Web servers publish jobs, workers consume them.*

### Java Example – Message Queue with RabbitMQ (Spring AMQP)

```java
// === Producer: Web server publishes photo processing job ===
@Service
public class PhotoPublisher {
    ...
    public void publishPhotoJob(String photoId, String operation) {
        Map<String, String> message = Map.of(
            "photoId", photoId,
            "operation", operation,  // "crop", "blur", "sharpen"
            "timestamp", Instant.now().toString()
        );
        rabbitTemplate.convertAndSend("photo-processing-queue", message);
        System.out.println("Published job for photo: " + photoId);
    }
}
```

```java
// === Consumer: Photo processing worker ===
@Component
public class PhotoWorker {

    @RabbitListener(queues = "photo-processing-queue")
    public void processPhoto(Map<String, String> message) {
        String photoId = message.get("photoId");
        ...
        switch (operation) {
            case "crop"    -> cropPhoto(photoId);
            case "blur"    -> blurPhoto(photoId);
            case "sharpen" -> sharpenPhoto(photoId);
        }

        System.out.println("Completed processing photo: " + photoId);
    }

    private void cropPhoto(String id) { /* crop logic */ }
    private void blurPhoto(String id) { /* blur logic */ }
    private void sharpenPhoto(String id) { /* sharpen logic */ }
}
```
...
### Java Example – Metrics with Micrometer + Spring Boot Actuator
...
![Figure 19 – Updated Design with Queue and Tools](images/ch02/figure-19.png)

*Figure 19: Updated design with message queue, workers, NoSQL, and monitoring/logging tools.*
...
![Figure 20 – Vertical vs Horizontal Scaling](images/ch02/figure-20.svg)

*Figure 20: Vertical Scaling (increasing CPU, RAM, DISK) vs Horizontal Scaling (adding more servers).*
...
![Figure 21 – Sharding by user_id](images/ch02/figure-21.svg)

*Figure 21: Sharding by user_id % 4 – routing data to one of four database shards.*

Figure 22 shows the user table in sharded databases.

![Figure 22 – User Table in Shards](images/ch02/figure-22.svg)

*Figure 22: User table distributed across 4 shards. Shard 0 has user IDs 0, 4, ...; Shard 1 has 1, 5, ...; etc.*
...
### Java Example – Database Sharding Logic
...
public class ShardRouter {
    ...
    public ShardRouter(Map<Integer, DataSource> shardMap) {
        this.shardMap = shardMap;
        this.totalShards = shardMap.size();
    }

    /**
     * Determines the shard for a given user ID using modulo hashing.
     * user_id % 4 → shard number (0, 1, 2, or 3)
     */
    ...
    public static void main(String[] args) {
        // Example: determine which shard a user belongs to
        int totalShards = 4;

        long[] userIds = {0, 1, 2, 3, 4, 5, 6, 7, 12, 13};
        for (long userId : userIds) {
            int shard = (int) (userId % totalShards);
...
![Figure 23 – Final Architecture](images/ch02/figure-23.png)

*Figure 23: Final architecture with sharded databases, NoSQL, message queue, workers, CDN, and monitoring tools.*
...
[4] Multi-master replication: [https://en.wikipedia.org/wiki/Multi-master_replication](https://en.wikipedia.org/wiki/Multi-master_replication)

[5] NDB Cluster Replication: Bidirectional and Circular Replication: [https://dev.mysql.com/doc/refman/8.4/en/mysql-cluster-replication-multi-source.html](https://dev.mysql.com/doc/refman/8.4/en/mysql-cluster-replication-multi-source.html)
...
[7] R. Nishtala etc. al., "Scaling Memcache at Facebook," 10th USENIX Symposium on Networked Systems Design and Implementation (NSDI '13): [https://www.usenix.org/system/files/conference/nsdi13/nsdi13-final170_update.pdf](https://www.usenix.org/system/files/conference/nsdi13/nsdi13-final170_update.pdf)
...
[14] What The Heck Are You Actually Using NoSQL For: [http://highscalability.com/blog/2010/12/6/what-the-heck-are-you-actually-using-nosql-for.html](http://highscalability.com/blog/2010/12/6/what-the-heck-are-you-actually-using-nosql-for.html)
