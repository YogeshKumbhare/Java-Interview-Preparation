...
> Source: [ByteByteGo - System Design Interview](https://bytebytego.com/courses/system-design-interview/a-framework-for-system-design-interviews)
...
### Java Example – Requirement Gathering Template

```java
/**
 * Template class for documenting system design interview requirements.
 * Use this as a structured way to capture all clarifications.
 */
public class SystemDesignRequirements {

    // Functional Requirements
    private String systemName;
    private List<String> coreFeatures;
    private List<String> secondaryFeatures;
    ...
    private int writeLatencyMs;
    private double availabilitySLA; // e.g., 99.99

    // Storage Requirements
    private String dataRetentionPolicy;
    private long estimatedStoragePerUser;

    // Constraints / Assumptions
    private List<String> assumptions;

    public static void main(String[] args) {
        SystemDesignRequirements req = new SystemDesignRequirements();
        req.systemName = "News Feed System";
        req.coreFeatures = List.of("Create post", "View news feed");
        req.secondaryFeatures = List.of("Like", "Comment", "Share");
        req.dailyActiveUsers = 10_000_000L;
        req.availabilitySLA = 99.99;
        req.assumptions = List.of(
            "Feed sorted by reverse chronological order",
            "Max 5000 friends per user",
            "Feed supports text, images, and videos"
        );

        System.out.println("=== System Design Requirements ===");
        System.out.println("System: " + req.systemName);
        System.out.println("Core Features: " + req.coreFeatures);
        System.out.println("DAU: " + String.format("%,d", req.dailyActiveUsers));
        System.out.println("SLA: " + req.availabilitySLA + "%");
        System.out.println("Assumptions:");
        req.assumptions.forEach(a -> System.out.println("  - " + a));
    }
}
```
...
![Figure 1 – Feed Publishing Flow](images/ch04/figure-1.png)

*Figure 1: High-level design for feed publishing – User → Load Balancer → Web Servers → Post Service / Fanout Service / Notification Service.*

![Figure 2 – News Feed Building Flow](images/ch04/figure-2.png)

*Figure 2: High-level design for news feed building – User → Load Balancer → Web Servers → News Feed Service → News Feed Cache.*
...
![Figure 3 – Feed Publishing Deep Dive](images/ch04/figure-3.png)

*Figure 3: Detailed design for feed publishing – including Post Service, Fanout Service, Graph DB, User Cache, Message Queue, Fanout Workers, and News Feed Cache.*

![Figure 4 – News Feed Retrieval Deep Dive](images/ch04/figure-4.png)

*Figure 4: Detailed design for news feed retrieval – including News Feed Service, News Feed Cache, User Cache, Post Cache, and CDN.*
...
### Java Example – Interview Timer Utility
...
public class InterviewTimer {
    ...
    record Step(String name, int minMinutes, int maxMinutes) {}

    public static void main(String[] args) {
        List<Step> steps = List.of(
            new Step("Understand problem & scope", 3, 10),
            new Step("High-level design & buy-in", 10, 15),
            new Step("Design deep dive", 10, 25),
            new Step("Wrap up", 3, 5)
        );

        int totalMin = 0, totalMax = 0;
        System.out.println("=== 45-Minute System Design Interview Time Allocation ===");
        System.out.printf("%-40s %-15s%n", "Step", "Time Range");
        System.out.println("-".repeat(55));

        for (Step step : steps) {
            System.out.printf("%-40s %d - %d minutes%n",
                step.name(), step.minMinutes(), step.maxMinutes());
            totalMin += step.minMinutes();
            totalMax += step.maxMinutes();
        }

        System.out.println("-".repeat(55));
        System.out.printf("%-40s %d - %d minutes%n", "TOTAL", totalMin, totalMax);
    }
}
```
